...
// @response            
router.post("/create", authMiddleware, MatchesController.createMatches);

// @route {PUT}         /match/score
// @bodyparams          id(match), score: string(20) matching regex /^-?\d+\/-?\d+$/ (two ints separated by a /)
// @response 
router.put("/score", authMiddleware, MatchesController.scoreMatch);

// @route {DELETE}      /match/delete
// @bodyparams          id(match)
// @response 
router.delete("/delete", authMiddleware, MatchesController.deleteMatch);
